...
dotnet user-secrets set "AppSettings:GoogleClientId" "DIN_GOOGLE_CLIENT_ID.apps.googleusercontent.com"
...
* **`IdentitySeeder`:** Oppretter standard roller (`admin`, `user` — alltid små bokstaver) og en initial admin-konto (`admin@kjoekkenhylla.local`). Retter automatisk opp eldre `Admin`/`User`-rader til lowercase ved oppstart.


* **`OpenIddictSeeder`:** Oppretter godkjente OAuth2-klienter (`recipe-web-app` og `recipe-mobile-app`) og legger til manglende permissions på klienter som allerede finnes.
...
* **`AuthorizationController` (`/api/auth/connect`):** OpenIddict OAuth2/OIDC token-utstedelse (`password` og `refresh_token` grants) og token-inndragelse (`POST /revoke`, RFC 7009).
...
* **`InvalidEmailDetectedConsumer`:** Reagerer på uleverbare e-poster for automatisk svartelisting, token-inndragning og kontosletting.
...
1. **Enhetstester (MediatR Handlers):** Kjøres mot en ekte `UserManager`/`SignInManager`/`RoleManager` over en SQLite in-memory-database (ikke EF Core In-Memory — se testverktøy under for hvorfor), med NSubstitute-mocker kun for rene grensesnitt (`IPublishEndpoint`, `ITokenService`, m.fl.).
...
| **[`05-openiddict-security-and-jobs.md`](Documentation/05-openiddict-security-and-jobs.md)** | OAuth2/OIDC token-utstedelse (Password & Refresh grant), token-inndragelse (`/connect/revoke`), Google OAuth2 callback-flyt (inkl. hvordan den utsteder ekte OpenIddict-tokens uten en levende HTTP-forespørsel), og Quartz.NET `AccountLifecycleJob`.
